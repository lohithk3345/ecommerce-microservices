...
type CategoryStore[T interface{}] interface {
...
type CategoryDatabase[T interface{}] struct {
	collection *mongo.Collection
}

func NewCategoryDatabase[T interface{}](database *mongo.Database, collection string) *CategoryDatabase[T] {
	return &CategoryDatabase[T]{
		collection: database.Collection(collection),
	}
}

func (d *CategoryDatabase[T]) insertOne(object T) (types.CategoryID, error) {
...
func (d *CategoryDatabase[T]) findOne(filter interface{}) (*mongo.SingleResult, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
...
func (d *CategoryDatabase[T]) updateOne(filter interface{}, update interface{}) (reporesult.InsertResult, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
...
func (d *CategoryDatabase[T]) deleteOne(filter interface{}) (reporesult.InsertResult, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
...
func (d *CategoryDatabase[T]) find(ctx context.Context, filter interface{}) (*mongo.Cursor, error) {
