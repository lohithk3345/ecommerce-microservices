...
type CartStore[T interface{}] interface {
...
type CartDatabase[T interface{}] struct {
	collection *mongo.Collection
}

func NewCartDatabase[T interface{}](database *mongo.Database, collection string) *CartDatabase[T] {
	return &CartDatabase[T]{
		collection: database.Collection(collection),
	}
}

func (d *CartDatabase[T]) insertOne(object T) (types.ID, error) {
...
func (d *CartDatabase[T]) findOne(filter interface{}) (*mongo.SingleResult, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
...
func (d *CartDatabase[T]) updateOne(filter interface{}, update interface{}) error {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
...
func (d *CartDatabase[T]) deleteOne(filter interface{}) (reporesult.InsertResult, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
...
func (d *CartDatabase[T]) find(ctx context.Context, filter interface{}) (*mongo.Cursor, error) {
